    ...
    def CheckRith(self):
        if self.RightEnable:
            if self.get_current_user() == '1':
                return True
            try:
                Rights = RightsCache.get("User%sRight" %
                ...
                for key in Rights.keys():
                    if re.match('^{0}$'.format(key), url.split('?')[0]) == None:
                        ...
                        if self.request.method.lower() in Rights[key]:
                            ...
                            self.finish(
                                {'errorMsg': '权限不足，不能进行该操作！', 'errorno': 10000, 'success': False})
                self.finish({'errorMsg': '权限不足，不能进行该操作！',
                             'errorno': 10000, 'success': False})
            except TypeError, e:
                self.finish({'errorMsg': '登陆过期，请重新登陆！',
                             'errorno': 10001, 'success': False})
                return False
